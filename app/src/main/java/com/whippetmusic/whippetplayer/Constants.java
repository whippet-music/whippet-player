package com.whippetmusic.whippetplayer;

/**
 * Created by maciej on 22.05.17.
 */

public abstract class Constants {
    public static final String API_URL = "http://10.0.2.2:5000";
    public static final String SHARED_PREFERENCES = "sharedPreferences";
    public static final String TOKEN_KEY = "accessToken";
    public static final String LOGISTIC_WEIGHTS_FILENAME = "local_knowledge";
    public static final int WEIGHTS_LENGTH = 18;
    public static final int RECOMMENDATION_MAX = 5;
}
